...
const FashionStyle = () => {
  ...
  const [Season, setSeason] = useState(""); // Search query for title and city
  ...
  // Filtered city options (excluding first 4 as per your code)
  ...
  useEffect(() => {
    ...
    if (subCatgory === "Perfumes ") {
      setsubCatgory("Perfumes & Incense");
    } else if (subCatgory === "Children's Clothing") {
      setsubCatgory("Children's Clothing & Accessories");
    } else if (subCatgory === "Health") {
      setsubCatgory("Health & Beauty");
    } else {
  ...
  const toggleBookmark = async (carId) => {
    try {
      ...
      // Update Firestore
      const carDocRef = doc(db, "FASHION", carId);
  ...
  //       console.log(data, "fashionData_________");
  //     } catch (error) {
  //       console.error("Error getting fashion data:", error);
  //       setLoading(false);
  //     }
  //   };
  ...
  useEffect(() => {
    ...
    const fetchFashion = async () => {
      try {
        ...
        setCars(fashionData);
        setFilteredCars(fashionData);
        setLoading(false);

        console.log(fashionData, "fashionData_________");
      } catch (error) {
        console.error("Error getting FASHION:", error);
        setLoading(false);
      }
    };
                              ...
                              >
                                ...
                                <div className="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-lg">
                                  <div className="modal-content border-0 shadow-lg">
                                    {/* Header with search */}
                                    <div className="modal-header bg-light border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
                                    ...
                                    <div className="modal-footer bg-light border-top d-flex justify-content-between align-items-center">
                                      <div className="text-muted small">
                                        {selectedRegion.length > 0
                                          ? `${selectedRegion.length} region(s) selected`
                                          : "No region selected"}
                                      </div>
