...
const PetAnimalsComp = () => {
  ...
  useEffect(() => {
    const fetchAdsDetailImages = async () => {
      try {
        const adsCollectionRef = collection(db, "PetAnimalsContent");
  ...
  useEffect(() => {
    ...
    const fetchPets = async () => {
      try {
        ...
        setCars(petsData);
        setFilteredCars(petsData);
        setLoading(false);

        console.log(petsData, "petsData_________PETANIMALCOMP");
      } catch (error) {
        console.error("Error getting PETANIMALCOMP:", error);
        setLoading(false);
      }
    };
