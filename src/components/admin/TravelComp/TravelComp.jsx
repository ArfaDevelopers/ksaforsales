...
const currentUserId = user?.uid;
  ...
  useEffect(() => {
    ...
    if (subCatgory?.trim() === "Accounting") {
      setsubCatgory("Accounting & Financial Services");
    } else {
  ...
  const handleCheckboxChangeRoomType = (event) => {
    ...
    if (event.target.checked) {
      ...
      setRoomType((prevRoomType) => [...prevRoomType, roomLabel]);
    } else {
  ...
  const handleCheckboxChangeCheckin = (event) => {
    const carLabel = event.target.name; // Use the name attribute to identify the checkbox
    if (event.target.checked) {
      // Add the selected checkbox to the array
      setCheckin((prevAmenities) => [...prevAmenities, carLabel]);
    } else {
      // Remove the unchecked checkbox from the array
  ...
  const handleCheckboxChangeisFeatured = (event) => {
    ...
    console.log(`Selected Option:____ 1${value}`);
  };
  ...
  const toggleBookmark = async (carId) => {
    try {
      ...
      // Update Firestore
      const carDocRef = doc(db, "TRAVEL", carId);
  ...
  const filterCars = (
    ...
    if (query.trim() !== "") {
      ...
      filtered = filtered.filter(
        (car) =>
          ...
          car.Checkin?.toLowerCase().includes(lowercasedQuery) ||
    ...
    if (RoomType?.length > 0) {
      filtered = filtered.filter((car) => RoomType.includes(car.RoomType));
    }
    ...
    if (Checkin?.length > 0) {
      filtered = filtered.filter((car) => Checkin.includes(car.Checkin));
    }
                ...
                <Row className="my-3">
                  <Col>
                    ...
                    <div className="position-relative">
                      <input
                        type="search"
                        placeholder="E.g Hotel Booking  in America"
  ...
  `}</style>
                  ...
                  <Accordion.Item eventKey="0">
                    ...
                    <Accordion.Body>
                      <Form.Group className="mb-3">
                        {/* Checkbox Selection */}
                        <div style={{ maxWidth: "300px", marginTop: "20px" }}>
                          {["Specific dates", "flexible date options"].map(
                            (car, index) => (
                              <div
                                key={index}
                                style={{
                ...
                <Accordion className="mt-3">
                  <Accordion.Item eventKey="0">
                    <Accordion.Header>Room Type</Accordion.Header>
                    <Accordion.Body>
                      <div style={{ maxWidth: "300px", margin: "20px" }}>
                        <Form.Group>
                          {["Single", "Double", "Family room"].map(
                            (engine, index) => (
                              <div
                                key={engine}
                                style={{
                              ...
                              >
                                <Form.Check
                                  type="checkbox"
                                  label={engine}
                                  name={engine} // Add the name attribute here
                                  onChange={handleCheckboxChangeRoomType} // No need for inline function
                                />
                                <span
                                  style={{ fontWeight: "bold", color: "#333" }}
                                >
                ...
                <Accordion className="mt-3">
                  <Accordion.Item eventKey="0">
                    ...
                    <Accordion.Body>
                      <div style={{ maxWidth: "300px", margin: "20px" }}>
                        <Form.Group>
                          {["Free Wi-Fi", "pool", "gym", "parking"].map(
                            (engine) => (
                              <div
                                key={engine}
                                style={{
                            ...
                            (engine) => (
                              ...
                              >
                                <Form.Check
                                  ...
                                  label={engine}
                                  name={engine} // Set the name attribute
                                  checked={PropertyType.includes(engine)} // Control the checked state
                                  onChange={handleCheckboxChangePropertyType} // Pass the event object
                                />
                                <span
                                  style={{ fontWeight: "bold", color: "#333" }}
                                >
        ...
        >
          <div className="cars data">
            ...
            <h2>Hotel Bookings in Newyork</h2>
            <p>
              Lorem ipsum dolor sit amet consectetur. Lacus lacus est praesent
              gravida quam urna arcu integer. Semper vitae velit sed quisque
              felis sed in. Quis vulputate euismod consequat feugiat vulputate
            ...
            <h2>Browse More Hotel Booking in USA</h2>
            <p style={{ color: "#2d4fad" }}>
              View more Hotel Booking by Cities
            </p>
