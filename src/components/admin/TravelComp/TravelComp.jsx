...
import { storage } from "../../Firebase/FirebaseConfig"; // Ensure the correct Firebase import
...
const TravelComp = () => {
  ...
  useEffect(() => {
    const fetchAdsDetailImages = async () => {
      try {
        const adsCollectionRef = collection(db, "ServicesContent");
        const adsSnapshot = await getDocs(adsCollectionRef);
  ...
  useEffect(() => {
    ...
    const fetchTravel = async () => {
      try {
        ...
        setCars(travelData);
        setFilteredCars(travelData);
        setLoading(false);

        console.log(travelData, "travelData_________TRAVEL");
      } catch (error) {
